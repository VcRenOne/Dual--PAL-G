...
class M_AdapterLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, d_ff, dropout, add_gate=True, gate = None):
        ...
        # self.feed_forward = Adapter(embed_dim, d_ff)
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            cross_attn_output = self.self_attn(x, y, y)
        else:
            cross_attn_output = self.self_attn(x, x, x)

        if  self.add_gate:
            gate1 = torch.sigmoid(self.gate(self_attn_output[0]))
            gate1 = torch.mean(gate1, dim=1).unsqueeze(-1)
            x = self.norm1(x + self.dropout(self_attn_output[0] * gate1) + self.dropout(cross_attn_output[0]* (1-gate1)))
        else:
            x = self.norm1(x + self.dropout(self_attn_output[0]) + self.dropout(cross_attn_output[0]))
...
class dual_prompt_Madapter_lora_onegate(TrainerX):
    ...
    def plot_tsne(self, features, labels, out_path, seed):
        ...
        features =  pca_50.fit(features)
        tsne = TSNE(n_components=3, init='pca', random_state=42, perplexity=50, learning_rate=500, n_iter=1000)
        class_num = len(np.unique(labels))  # 要分类的种类个数  eg:[0, 1, 2, 3]这个就是为4

        tsne_features = tsne.fit_transform(features)  # 将特征使用PCA降维至3维

        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')

        ax.scatter(tsne_features[:, 0], tsne_features[:, 1], tsne_features[:, 2], s=5, c=y_subset, cmap='Spectral')
        plt.title('t-SNE in 3D', fontsize=24);
        ...
        #                 palette=sns.color_palette("Set1", class_num),
